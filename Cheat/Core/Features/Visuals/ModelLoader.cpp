...
namespace Cheat::Core {
                 ...
                 std::vector<float>* out_unique_positions)
    {
                ...
                auto emit = [&](Idx a, Idx b, Idx c) {
                    auto vv = [&](Idx i) {
                        ...
                        if (i.v * 3 + 2 < (int)positions.size()) {
                            mv.x = positions[i.v * 3 + 0];
                            mv.y = positions[i.v * 3 + 1];
                            mv.z = positions[i.v * 3 + 2];
                        }
        ...
        out_center[0] = (mn[0] + mx[0]) * 0.5f;
        out_center[1] = (mn[1] + mx[1]) * 0.5f;
        out_center[2] = (mn[2] + mx[2]) * 0.5f;

        if (out_unique_positions) {
            *out_unique_positions = positions;
        }
