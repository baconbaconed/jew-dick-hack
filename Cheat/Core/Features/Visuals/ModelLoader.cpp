...
namespace Cheat::Core {
    namespace {
        ...
        void ExpandAABB(ModelPartAABB& a, float x, float y, float z)
        {
            ...
            a.min[0] = (std::min)(a.min[0], x); a.max[0] = (std::max)(a.max[0], x);
            a.min[1] = (std::min)(a.min[1], y); a.max[1] = (std::max)(a.max[1], y);
            a.min[2] = (std::min)(a.min[2], z); a.max[2] = (std::max)(a.max[2], z);
        }
        ...
        bool IsWingLikeAccessory(const ModelPartAABB& a, float bodyW, float bodyH)
        {
            ...
            const float wingW = (std::max)(3.2f, bodyW * 1.25f);
            const float wingD = (std::max)(2.4f, bodyW * 1.05f);
            if (ex >= wingW) return true;
            if (ez >= wingD && ez > ey * 0.85f) return true;
            if (ex >= bodyH * 0.85f && ex > ey * 1.4f) return true;
            return false;
        }
        ...
        void FinalizeModel(LoadedModel& out, std::vector<GroupAccum>& groups)
        {
            ...
            const float bodyW = (std::max)(0.01f, out.body_max[0] - out.body_min[0]);
            const float bodyH = (std::max)(0.01f, out.body_max[1] - out.body_min[1]);

            for (auto& g : groups) {
                const bool wing = g.handle && IsWingLikeAccessory(g.aabb, bodyW, bodyH);
                if (wing) {
            ...
            const float ext[3] = {
                out.raw_max[0] - out.raw_min[0],
                out.raw_max[1] - out.raw_min[1],
                out.raw_max[2] - out.raw_min[2]
            };
            ...
            while (std::getline(stream, line)) {
                ...
                if (s[0] == 'v' && s[1] == ' ') {
                    ...
                    while (*p && count < 8) {
                        ...
                        if (vi > 0) idx[count++] = { vi - 1, vti - 1, vni - 1 };
                        else break;
                    }
