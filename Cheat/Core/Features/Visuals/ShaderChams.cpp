...
namespace Cheat {
    namespace Visuals {
        namespace ShaderChams {
            namespace {
                ...
                std::vector<ImVec2> ClipHalf(const std::vector<ImVec2>& in, Side side, Intersect isect) {
                    ...
                    for (int i = 0; i < n; ++i) {
                        ...
                        const bool cin = side(cur)  >= 0.0f;
                        const bool pin = side(prev) >= 0.0f;
                        if (cin) {
                            if (!pin) out.push_back(isect(prev, cur));
                            out.push_back(cur);
                        } else if (pin) {
                ...
                std::vector<ImVec2> ClipDiagBand(const std::vector<ImVec2>& poly, float d0, float d1) {
                    ...
                    auto lo = ClipHalf(poly,
                        ...
                        [d0](const ImVec2& a, const ImVec2& b) {
                            const float da = a.x + a.y, db = b.x + b.y;
                            const float t = (d0 - da) / (db - da + 1e-6f);
                    ...
                    return ClipHalf(lo,
                        ...
                        [d1](const ImVec2& a, const ImVec2& b) {
                            const float da = a.x + a.y, db = b.x + b.y;
                            const float t = (d1 - da) / (db - da + 1e-6f);
                            return ImVec2(a.x + (b.x - a.x) * t, a.y + (b.y - a.y) * t);
                        });
                }
                ...
                void RunAnim(ImDrawList* dl,
                    ...
                    const float phase = std::fmod(time * def.speed, 1.0f);
                    const ImU32 band = MulAlpha(pal.band, pulse_a);
                    const ImU32 core = MulAlpha(pal.band_core, pulse_a);
                    const float bf = (std::clamp)(def.band_frac, 0.03f, 0.5f);
                    ...
                    case AnimKind::Wave: {
                        ...
                        const float seg_w = w / (float)segs;
                        const float bh = h * bf;
            ...
            ImU32 OutlineColor(int style, bool aim_highlight) {
                if (aim_highlight) return AimPalette().outline;
                style = (std::clamp)(style, 0, StyleCount - 1);
                if (style == Chromatic)
                    return HSVA(std::fmod(ImGui::GetTime() * 0.35f, 1.0f), 0.85f, 1.0f, 0.90f);
            ...
            void Triangulate(const std::vector<ImVec2>& poly,
                ...
                if (n0 < 3) return;
                if (n0 == 3) { out_tris.push_back(poly); return; }
                ...
                auto point_in_tri = [&](const ImVec2& p, const ImVec2& a,
                    ...
                    const float a0 = area2(a, b, p);
                    const float a1 = area2(b, c, p);
                    const float a2 = area2(c, a, p);
                    const bool has_neg = (a0 < 0) || (a1 < 0) || (a2 < 0);
                    const bool has_pos = (a0 > 0) || (a1 > 0) || (a2 > 0);
                ...
                while ((int)idx.size() > 3 && guard-- > 0) {
                    ...
                    for (int i = 0; i < m; ++i) {
                        ...
                        if (area2(a, b, c) <= 0.0f) continue;

                        bool ear = true;
                        for (int j = 0; j < m; ++j) {
                            const int ij = idx[j];
                            if (ij == i0 || ij == i1 || ij == i2) continue;
                            if (point_in_tri(poly[ij], a, b, c)) { ear = false; break; }
                        }
                        if (!ear) continue;
            ...
            void DrawFill(ImDrawList* draw_list,
                          ...
                          bool aim_highlight,
                          bool lite) {
                if (!draw_list || pieces.empty()) return;
                style = (std::clamp)(style, 0, StyleCount - 1);
                ...
                for (const auto& p : pieces) {
                    ...
                    ImVec2 mn, mx;
                    Bounds(p, mn, mx);
                    if (!have_bounds) { body_mn = mn; body_mx = mx; have_bounds = true; }
                    else {
                        body_mn.x = (std::min)(body_mn.x, mn.x);
                        body_mn.y = (std::min)(body_mn.y, mn.y);
                        body_mx.x = (std::max)(body_mx.x, mx.x);
                        body_mx.y = (std::max)(body_mx.y, mx.y);
                    }
                }
                ...
                (void)lite;
                Palette pal = aim_highlight ? AimPalette() : def.pal;

                if (!aim_highlight && style == Chromatic) {
                    const float h = std::fmod(time * def.speed * 0.35f, 1.0f);
                ...
                const float fresnel = (std::clamp)(def.fresnel, 0.0f, 1.0f);
                const ImU32 top = MulAlpha(pal.top, pulse_a * (0.55f + 0.45f * fresnel));
                const ImU32 bot = MulAlpha(pal.bot, pulse_a);
