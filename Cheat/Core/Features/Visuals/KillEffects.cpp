        ...
        namespace KillEffects {
            namespace {
                ...
                ImU32 MulA(ImU32 c, float a) {
                    const int na = (int)((float)((c >> IM_COL32_A_SHIFT) & 0xFF) * a + 0.5f);
                    return (c & ~IM_COL32_A_MASK) | ((ImU32)(std::clamp)(na, 0, 255) << IM_COL32_A_SHIFT);
                }

                ImU32 LerpU32(ImU32 a, ImU32 b, float t) {
                    t = (std::clamp)(t, 0.0f, 1.0f);
                    const auto ch = [](ImU32 c, int sh) { return (int)((c >> sh) & 0xFF); };
                    const int r = (int)(ch(a, IM_COL32_R_SHIFT) + (ch(b, IM_COL32_R_SHIFT) - ch(a, IM_COL32_R_SHIFT)) * t);
                    const int g = (int)(ch(a, IM_COL32_G_SHIFT) + (ch(b, IM_COL32_G_SHIFT) - ch(a, IM_COL32_G_SHIFT)) * t);
                    const int bl = (int)(ch(a, IM_COL32_B_SHIFT) + (ch(b, IM_COL32_B_SHIFT) - ch(a, IM_COL32_B_SHIFT)) * t);
                    const int al = (int)(ch(a, IM_COL32_A_SHIFT) + (ch(b, IM_COL32_A_SHIFT) - ch(a, IM_COL32_A_SHIFT)) * t);
                ...
                void SpawnKill(int type, const Vector3& origin) {
                    ...
                    switch (type) {
                        case Confetti:   fx.life = 1.55f; break;
                        case Shatter:    fx.life = 1.20f; break;
                        case PixelBurst:
                        default:         fx.life = 1.30f; break;
                    }
                    g_fx.push_back(fx);
                    if (g_fx.size() > 20)
                ...
                void SpawnMarker(const Vector3& origin, bool headshot) {
                    ...
                    m.origin = origin;
                    m.age = 0.0f;
                    m.life = (std::max)(0.2f, g_Settings.hitmarker.duration);
                    m.headshot = headshot;
                ...
                void SpawnHitTexts(const Vector3& origin, int part, float damage,
                    ...
                    const bool headshot = IsHead(part);
                    int line = 0;
                    for (int m = 0; m < Settings::HITDATA_MODE_COUNT; ++m) {
                        if (!g_Settings.hitdata.modes[m]) continue;

                        HitText t;
                        t.origin = origin;
                        t.origin.y += (float)line * 0.45f;
                        t.age = 0.0f;
                        t.life = (std::max)(0.35f, g_Settings.hitdata.duration);
                        t.headshot = headshot;
                ...
                void DrawPixelBurst(ImDrawList* dl, const KillFx& fx, const Matrix4x4& vm,
                    ...
                    for (int r = 0; r < 2; ++r) {
                        const float u = (std::clamp)(t * 1.25f - r * 0.18f, 0.0f, 1.0f);
                ...
                void DrawShatter(ImDrawList* dl, const KillFx& fx, const Matrix4x4& vm,
                    ...
                    for (int r = 0; r < 3; ++r) {
                        const float u = (std::clamp)(t * 1.5f - r * 0.12f, 0.0f, 1.0f);
                ...
                void DrawKillFx(ImDrawList* dl, const KillFx& fx, const Matrix4x4& vm,
                    ...
                    const float t = (std::clamp)(fx.age / fx.life, 0.0f, 1.0f);
                    switch (fx.type) {
                        case Confetti: DrawConfetti(dl, fx, vm, vp, sx, sy, t); break;
                        case Shatter:  DrawShatter(dl, fx, vm, vp, sx, sy, t); break;
                        case PixelBurst:
                        default:       DrawPixelBurst(dl, fx, vm, vp, sx, sy, t); break;
                    }
                }

                void DrawHitmarker(ImDrawList* dl, const Marker& m, const Matrix4x4& vm,
                                   const Vector2& vp, float sx, float sy) {
                    const float t = (std::clamp)(m.age / m.life, 0.0f, 1.0f);
                    const float fade_in = (std::clamp)(t / 0.08f, 0.0f, 1.0f);
                    const float fade_out = 1.0f - t;
                    const float a = fade_in * fade_out;
                ...
                void DrawHitText(ImDrawList* dl, const HitText& ht, const Matrix4x4& vm,
                                 const Vector2& vp, float sx, float sy) {
                    const float t = (std::clamp)(ht.age / ht.life, 0.0f, 1.0f);
                    const float fade_in = (std::clamp)(t / 0.08f, 0.0f, 1.0f);
                    const float fade_out = 1.0f - (std::clamp)((t - 0.6f) / 0.4f, 0.0f, 1.0f);
                    const float a = fade_in * fade_out;
                    if (a <= 0.01f) return;
            ...
            void Tick() {
                const bool any = g_Settings.killfx.enabled
                ...
                const std::uint64_t target = Features::Aim::CurrentTarget();
                const bool lmb = (GetAsyncKeyState(VK_LBUTTON) & 0x8000) != 0;
                if (lmb && !g_lmb_was)
                    g_lmb_at = now;
                g_lmb_was = lmb;

                const bool recent_shot = lmb || (g_lmb_at >= 0.0 && (now - g_lmb_at) < 0.45);

                const int aim_part = Features::Aim::CurrentAimPart();
                ...
                if (g_Settings.killfx.enabled) {
                    for (size_t i = 0; i < g_pending.size(); ) {
                        ...
                        float hp = 0.0f;
                        bool found = false;
                        const bool has_pos = LookupTarget(p.addr, pos, hp, found);
                        if (has_pos) p.pos = pos;

                        const bool dead = !found || hp <= 0.0f;
                        if (p.was_alive && dead) {
                ...
                const uintptr_t ve = g_Memory.Read<uintptr_t>(s_base + Offsets::VisualEngine::Pointer);
                const Matrix4x4 vm = g_Memory.Read<Matrix4x4>(ve + Offsets::VisualEngine::ViewMatrix);
