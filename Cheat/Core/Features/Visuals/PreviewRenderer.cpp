...
bool PreviewRenderer::CreateRenderTarget(unsigned int w, unsigned int h)
{
    ...
    if (FAILED(m_Device->CreateTexture2D(&rd, nullptr, &m_ResolveTex))) return false;
    if (FAILED(m_Device->CreateShaderResourceView(m_ResolveTex, nullptr, &m_SRV))) return false;
...
void PreviewRenderer::BuildR6SkeletonFromParts()
{
    ...
    auto add = [&](float ax, float ay, float az, float bx, float by, float bz) {
        if (m_SkelSegCount >= k_MaxSkelSegs) return;
    ...
    lerp3(torso_top, torso_bot, kShoulderDrop, shoulder_c);
...
{
    m_ManualYaw += dyaw;
    m_ManualPitch = (std::clamp)(m_ManualPitch + dpitch, -1.2f, 1.2f);
    m_SceneDirty = true;
}

void PreviewRenderer::AddZoom(float delta)
{
    m_Zoom = (std::clamp)(m_Zoom + delta, 0.45f, 2.8f);
    m_SceneDirty = true;
}
...
float PreviewRenderer::WingAlpha() const
{
    ...
    constexpr float kFadeIn  = 1.15f;
    constexpr float kHold    = 1.60f;
    constexpr float kFadeOut = 1.15f;
    constexpr float kPause   = 0.90f;
    constexpr float kPeriod  = kFadeIn + kHold + kFadeOut + kPause;

    float t = std::fmod(m_WingAnimTime, kPeriod);
    if (t < 0.0f) t += kPeriod;
    ...
    if (t < kFadeIn)
        return smooth(t / kFadeIn);
    t -= kFadeIn;
    if (t < kHold)
        return 1.0f;
    t -= kHold;
    if (t < kFadeOut)
        return 1.0f - smooth(t / kFadeOut);
    return 0.0f;
}

void PreviewRenderer::Update(float dt)
{
    if (!m_Ready || !m_VB) return;
    ...
    const float camDist = 2.35f / m_Zoom;
