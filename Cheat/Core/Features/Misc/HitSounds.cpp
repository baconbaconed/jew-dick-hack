...
namespace {
    ...
    void ApplyVolume(std::vector<BYTE>& pcm16, float volume)
    {
        ...
        const std::size_t count = pcm16.size() / sizeof(std::int16_t);
        for (std::size_t i = 0; i < count; ++i) {
            const float s = static_cast<float>(samples[i]) * volume;
            if (s > 32767.0f) samples[i] = 32767;
            else if (s < -32768.0f) samples[i] = -32768;
            else samples[i] = static_cast<std::int16_t>(s);
        }
    }
...
namespace Cheat::Features::HitSounds {
    ...
    void Play(int index)
    {
        ...
        const float vol_pct = g_Settings.hitsound.volume;
        if (vol_pct <= 0.0f)
            return;
        ...
        const float volume = (std::min)(vol_pct, 100.0f) / 100.0f;
